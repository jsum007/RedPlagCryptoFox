...
def tokenize(filename):
    ...
    for i in range(lenT):
        #print(tokens[i])
        if tokens[i][0] in pygments.token.Name and not i == lenT - 1 and not tokens[i + 1][1] == '(':
            #result.append(('N', count1, count2))  #all variable names as 'N'
            print(tokens[i][0])
            if tokens[i][0] in pygments.token.Name.Builtin or tokens[i][0] in pygments.token.Name.Function \
